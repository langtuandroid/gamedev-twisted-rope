...
namespace _Twisted._Scripts.ControllerRelated
{
    public class UIController : MonoBehaviour
    {
        ...
        {
            if(newState==GameState.Levelwin) DOVirtual.DelayedCall(2.5f, () => winPanel.SetActive(true));
            if(newState==GameState.Levelfail)
            {
                DOVirtual.DelayedCall(0.75f, () =>
                {
                    if(SceneManager.GetActiveScene().buildIndex < 4) failPanelNonPowerup.SetActive(true);
